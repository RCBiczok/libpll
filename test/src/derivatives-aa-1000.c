...
/*
    derivatives-aa-1000.c

    This test is similar to "derivatives-aa.c", but with a huge amount fo sites instead.
...
int main(int argc, char *argv[]) {
  ...
  for (k = 0; k < NUM_CATS; ++k) {
    ...
    const char *source_align[] = {"PIGLRVTLRRDRMWIPIGLR",
                                  "IQGMDITIVT-----IQGMD",
                                  "--AFALLQKIGMPFE--AFA",
                                  "MDISIVT------TAMDISI",
                                  "GLSEQTVFHEIDQDKGLSEQ"};
    ...
    for (size_t i = 0; i < align_seqs; i++) {
      ...
      for (size_t k = 0; k < n_sites; k++) {
        align[i][k] = source_align[i][k % strlen(source_align[i])];
      }
